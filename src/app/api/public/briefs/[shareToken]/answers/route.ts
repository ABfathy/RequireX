...
import { PublicFollowUpAnswerInputSchema } from "@/server/validators";
...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const parsed = PublicFollowUpAnswerInputSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const answer = await createPublicFollowUpAnswer(shareToken, parsed.data);
